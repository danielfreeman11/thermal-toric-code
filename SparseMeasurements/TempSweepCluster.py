...
for Patch_Size in [12, 18, 21]:
    #for Num_Of_Lambdas in [2,3,4,6,8,12,16,24,32]:
    for Num_Of_Lambdas in [32]:
        for Temperature_ in sorted(numpy.linspace(.07,.2,14), reverse = True):
...
PS, NumOfLambdas, Temperature = ParamList[int(sys.argv[1])%42]
...
print Temperature,"\t",str(PS),"\t",NumOfLambdas,"\t",SystemLength,"\t",CorrectionRate,"\t",numpy.mean(fliptimes),"\t",numpy.std(fliptimes)
