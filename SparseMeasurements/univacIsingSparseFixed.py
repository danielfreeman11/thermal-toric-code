...
NumOfLambdasList = [4,6,8,10,12,14,16]
NumOfLambdasList = [18,20,22,24,26,28] 
for outer_index in xrange(4):
	...
	#NumOfLambdasList = [2,3,4,6,8,12,16,24,32] 
	...
	def WindowMeasurementsCareful(chain, oldtime, newtime, CorrectionRate, PHop, PAnn, PCre, CorrectionSwaps, sector):
		...
		while(oldtime+CycleTime < newtime and not(ProbabilityHasChanged) and not(PHop == 0) and Proceed == True):# \
			...
			for k in xrange(NumOfLambdas):
				...
				if chain[(7*k-3)%SystemLength]==1 and chain[(7*k-2)%SystemLength]==0 and chain[(7*k-1)%SystemLength]==0:
					chain[(7*k-2)%SystemLength] = chain[(7*k-4)%SystemLength]
					chain[(7*k-4)%SystemLength] = 0
					
				if chain[(7*k-3)%SystemLength]==0 and chain[(7*k-2)%SystemLength]==0 and chain[(7*k-1)%SystemLength]==1:
					chain[(7*k-2)%SystemLength] = chain[(7*k)%SystemLength]
					chain[(7*k)%SystemLength] = 0
				...
				if chain[(7*k-3)%SystemLength]+chain[(7*k-2)%SystemLength]+chain[(7*k-1)%SystemLength]==1:
					#%#print "Centering!" + str(chain[(7*k-3)%SystemLength])+str(chain[(7*k-2)%SystemLength])+\
					#%#str(chain[(7*k-1)%SystemLength]) + " at center " + str((7*k-2)%SystemLength)
	...
	while(FlipHasOccurred == False):# and counter < 10000:# and totalt < exp(1./Temperature)):
		#if counter%1000==0: 
	...
	with open('/home/daniel.freeman/SparseMeasurements/Runs/' + str(seed) + '.' + str(Temperature) + '.' + str(outer_index) + '.sweep.dat','a+') as f:
	...
	print Temperature,"\t",NumOfLambdas,"\t",SystemLength,"\t",CorrectionRate,"\t",numpy.mean(fliptimes)
