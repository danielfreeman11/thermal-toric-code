...
def PerformRun(Dim, Temp, Delt, TranRate, CurTime, TotTime, NumOfRuns, NumOfPoints, Mod, FilNam):
    ...
    #choices = [(2,4),(4,4),(3,5),(1,3),(2,2),(3,1),(4,2),(5,3)]
    ...
    if(Mode == "TrivialSingleShot"):
        EndTimesAverage = 0
        NumberOfStepsAverage = 0
	WindingOperator = []
	for Run in xrange(NumberOfRuns):
            EndTimesAverage += SetOfTimesAndObservables[Run][0][-1]
	    NumberOfStepsAverage += len(SetOfTimesAndObservables[Run][0])
	    WindingOperator = SetOfTimesAndObservables[Run][1][-1]		

                
        EndTimesAverage = EndTimesAverage / (NumberOfRuns*1.0)
	NumberOfStepsAverage = NumberOfStepsAverage / (NumberOfRuns*1.0)
        TimeToRecombineAverage.append((double(EndTimesAverage),double(Dim),double(NumberOfRuns)))
        print str(EndTimesAverage) + "\t" + str(NumberOfStepsAverage) + "\t" + str(WindingOperator)
    ...
    if(Mode == "Coarse"):
		...
		FinalWinding = []
		if NumberOfRuns == 1:
			FinalWinding = SetOfTimesAndObservables[0][1][CurrentIndexes[0]]
                ...
                row = deque([WindingX1,WindingX2,NumOfExcitations,FinalWinding])
...
for LargeRun in xrange(1):
    ...
    Delt = 1.
    #TranRate = 1.0
    TranRate = 2.0 #Tranrate is actually being set by PerformRun
    CurTime = 0
    TotTime = 100000
    NumOfRuns = 1
    NumOfPoints = 100000
	...
	os.chdir("/home/daniel.freeman")
        FilNam = "FullLatticeRuns/" + str(Dim) + "_Temp" + str(Temp) + "_Delt" + str(Delt) + "_TransRate" + str(TranRate) + "_" +\
