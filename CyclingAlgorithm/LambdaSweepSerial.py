...
SystemLength = 96
NumOfLambdas = NumOfLambdasList[li]
...
def CorrectionProtocol(chain, oldtime, newtime, CorrectionRate, PHop, PAnn, PCre, CorrectionSwaps, sector):
    ...
    while(oldtime+CycleTime < newtime and not(ProbabilityHasChanged) and not(PHop == 0) and Proceed == True):# and not(PAnn > 0)):
        ...
        #    chain = CSwap(chain, CorrectionSwaps[(IndexInCycle + NumPar*2*BaseCycleLength)%NumberOfSwaps])  
        chain = CSwap(chain, CorrectionSwaps[(IndexInCycle)%NumberOfSwaps])
...
for i in xrange(1000):
    ...
    while(FlipHasOccurred == False):# and totalt < exp(1./Temperature)):
        ...
        '''    
        counter+=1
    with open('/home/daniel.freeman/ToricCodeProject2/inprogress/seriallambdasweep_fixed/run.'+str(sys.argv[1])+'.dat','a+') as f:
